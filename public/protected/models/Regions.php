...
class Regions extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('idRegion, name, description', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('idRegion, name, description', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'citys' => array(self::HAS_MANY, 'Citys', 'idRegion'),
			'realEstates' => array(self::HAS_MANY, 'RealEstate', 'idRegion'),
			'users' => array(self::HAS_MANY, 'Users', 'idRegion'),
		);
	}
	...
	public static function getIdRegionByName($nameRegion)
	{
		...
		$region		= self::model()->findByAttributes(
			array('name' => $nameRegion),
			array('select' => 'idRegion')	
		);
